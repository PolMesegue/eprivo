...
$sql = "SELECT domain.name, COUNT(domain_url.third_party) AS num, domain.intrusion_level FROM domain JOIN domain_url ON domain.id = domain_url.domain_id JOIN url ON domain_url.url_id = url.id WHERE domain_url.third_party = 1 GROUP BY domain.id ORDER BY num DESC limit 20";
